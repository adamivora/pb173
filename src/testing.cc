...
std::string to_string(const std::vector<unsigned char> &bytes) {
  ...
  for (const auto &byte : bytes) {
    stream << std::setfill('0') << std::setw(2) << std::hex << (int)byte;
  }

  return stream.str();
}

TEST_CASE("SHA-512 valid") {
  SHA sha;
  ...
  REQUIRE(
      ...
      to_string(sha.hash(very_long)));
}

TEST_CASE("SHA-512 invalid") {
  SHA sha;
